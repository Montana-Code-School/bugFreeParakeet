...
class StorySubmit extends Component {
  ...
  onSubmit(e){ //function for submit button
    if(this.state.optionsBox.length !== 0
      && this.state.premiseBox.length !== 0
      && this.state.optionsBox[0] !== "."
      && this.state.premiseBox[0] !== "."
      && this.state.optionsBox[0] !== " "
      && this.state.premiseBox[0] !== " ") {
    ...
    if(this.state.checker === false){
      return (
        <div>
          ...
          <input
            value= {this.state.optionsBox}
            onChange = {(e) => this.onChange(e)}
            id="optionInput"
            type="text"
            maxlength="50" />
          <p className="count">
            Character Limit: 50 Current: {this.state.optionsBox.length}
          </p>
          <br />
          <label className="labels">Premise: </label>
          <textarea
            value={this.state.premiseBox}
            onChange = {(e) => this.onChange(e)}
            id="premiseInput"
            rows="10"
            cols="75"
          ...
          <p className="count">
            Character Limit: 1000 Current: {this.state.premiseBox.length}
          </p>
          <br />
          <button
